...
global lock does not apply. Coordinate only genuinely huge concurrent TLC runs.
Each local test runs as a transient service which imports the caller's
environment and arguments without recording their values in transient-unit
metadata, binds to the invoking agent scope, and uses
`KillMode=control-group`. Completion, cancellation, or agent exit therefore
cannot leave its kube-apiserver, etcd, or other child processes behind.
...
The remote account must have a running systemd user manager. Each run uses a
transient service with `KillMode=control-group`. When a run ends, systemd sends
`SIGTERM` to its processes and sends `SIGKILL` after one second. This cleanup
also reaches processes that changed their session or process group.
...
- `dev`: `core` + developer tools such as `1password-cli`, `ansible`, `go`, `tofu` (OpenTofu), `doctl`, `gh`, `aws`, `openstack`, `hugo`, `picocom`, Claude Code (native installer), Codex CLI (`@openai/codex`), Gemini CLI (`@google/gemini-cli`), and ESP tooling (`esptool` + `idf.py` bootstrap), plus Neovim bootstrap.
